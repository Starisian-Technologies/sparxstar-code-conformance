...
**Version:** 0.1  
**Status:** SPECIFIED  
**Scope:** All client-side implementations (JavaScript/React, WordPress plugins, games, WordPad)  
...
```
ONLINE → SLOW_ONLINE   navigator.onLine true but API response > 3s or 429 received
...
- Application-layer mutations (WordPad edits, game progress saves)

## What Blocks (requires ONLINE or SYNCING)

The following operations MUST block until the state returns to ONLINE:
...
- Dead-letter: Items that fail 3 flush attempts move to dead-letter (see dead-letter retention standard in standards-handbook.md)
...
- Dead-letter: `docs/standards-handbook.md` — Section 5 + dead-letter retention rule
