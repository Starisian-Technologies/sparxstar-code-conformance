...
|---|---|---|---|
| 1 | PHP / WordPress | doc + Layer 1 config shipped (Composer pkg `starisian-technologies/coding-standards`) | `phpcs.xml.dist`, `phpstan.neon` shipped; custom sniffs (Step 7) outstanding |
...
3. ✅ **Layer 1 configs** for the two biggest estates: PHP, then JS/TS. *(this PR — `composer.json`, `phpcs.xml.dist`, `phpstan.neon`, `@starisian-technologies/eslint-config`, `@starisian-technologies/tsconfig`)*
